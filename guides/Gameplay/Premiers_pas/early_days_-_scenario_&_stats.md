...
Chaque scénario de carrière a ses personnages, son intrigue, et... **ses mécaniques de gameplay spécifique**. Enfin, en réalité, le tout premier scénario jouable -URA Finale- brillait par son **absence de mécaniques**. Mais à partir de Unity Cup -le 2e scénario-, tous les scénarios auront des mécaniques qui leurs sont spécifiques. 
...
Vous avez besoin d'**un certain nombre de fans pour participer** à une course donnée, donc il est possible qu'avant les 2-3 première courses vous ayze besoin de faire une course non-obligatoire ou deux. Vous pouvez décider de la course comme bon vous semble.
...
De manière générale, **le tout dernier scénario disponible dans le jeu est toujours le plus puissant** (sur JP il y a eu 1 exception jusqu'à présent), et ce sera toujours celui que vous voudrez faire le plus souvent. URA Finale est un scénario notablement faible, sur lequel vous devez faire des compromis, mais plus tard des scénarios puissants permettront une vraie croissance des stats de vos uma en carrière.
...
- va sprinter **vite** en fin de course → **speed** :speed:
- va **accélérer** vivement vers cette vitesse de sprint en fin de course → **power** :power:
- va sprinter **longtemps** en fin de course → **stamina** :stamina:
- va **conserver son énergie** pendant son sprint de fin de course → **guts** :guts:
- va se **positionner** de manière optimale pendant la course, _avant_ son sprint final → **wits** :wit:

::: tip Description simplifiée

Plusieurs de ces stats ont en réalité **plusieurs rôles**, mais comme vous débutez ce n'est pas pertinent de vous embrouiller avec ça pour le moment. De plus, ces usages alternatifs évoluent à mesure que le jeu évolue. Ainsi, sur la version JP (4 ans et demi d'avance), les stats power :power:, stamina :stamina:, guts :guts: et wits :wit: sont impliquées dans **plusieurs mécaniques de course additionnelles**, en plus de leurs rôles principaux décrits à l'instant.

Sur la version globale, guts est utilisé dans les mécaniques de "Spot Struggle" et "Dueling". On en parlera pas beaucoup dans ce guide, pour ne pas vous embrouiller.

:::

**Améliorer les stats de votre uma sera votre principal objectif pendant sa carrière**, au travers de multiples entrainements. La plupart du temps, si vous n'avez pas de course obligatoire à réaliser dans l'immédiat, alors vous allez généralement vouloir vous entrainer, si raisonnablement possible, afin d'améliorer les stats.
...
- là maintenant tout de suite, **au lancement d'une carrière**, en sélectionnant des legacies (garanti), comme on va le voir dans un instant
