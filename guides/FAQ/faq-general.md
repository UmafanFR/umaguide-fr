...
### Comment accumuler rapidement des carats :carat: ?
...
- toutes les G3 :g3: → 2070 carats (30 par première victoire)
- toutes les G2 :g2: → 1800 carats (50 par première victoire)
- toutes les G1 :g1: → 3000 carats (100 par première victoire)
...
**(mettez un mot de passe sur votre compte, par pitié → [voici comment faire](/guides/Gameplay/Premiers_pas/early_days_-_before_starting.md#faites-le-data-link-avec-un-mot-de-passe))**
...
:::: details Non, y'a des très grandes chances que vos teams auto-select ne soient pas bonnes. Je vous explique le raisonnement général pour le faire vous-même :
...
On a aucune foutue idée de comment fonctionne exactement l'auto-select, et l'auto-select n'a sans doute aucune foutue idée de comment il fonctionne. On pensait qu'il essayait de maximiser le score du rank de votre team, pour avoir le nombre le plus élevé, mais on a aussi vu des gens qui subissent une **baisse de points en utilisant l'auto-select**. Finalement, tout ce qu'on sait de l'auto-select c'est que la plupart des gens qui demandent si leurs teams auto-select sont bonnes ont généralement des **teams éclatées au sol**. ~~C'est sans doute Tazuna qui le gère~~
...
Autrement dit, **une aptitude notée E peut donc passer A de manière garantie** avec suffisamment d'inspiration. Une aptitude F ira jusqu'à B. N'hésitez pas à chercher des legacy qui vous arrangent, sur [chronogenesis.net](https://chronogenesis.net/friend_search), [uma.moe](https://uma.moe/inheritance) ou [umapureDB](https://uma-global.pure-db.com/#/search) pour trouver .
...
**On fait comme on peut, _faute de mieux_**, et on remplace par une uma avec un kit plus cohérent, quand c'est possible. Une uma qui n'active que 2 de ses 3 principaux skills c'est mieux que pas de uma, ou qu'une uma qui n'a pas l'aptitude, positionnée par l'auto-select. On est aux débuts de la version globale du jeu, **c'est parfaitement normal de devoir faire des tas de compromis**.
